...
<Aside type="note">
  Svelte Reveal wraps the target element in a `<div>` so it can apply transforms without polluting the target's own
  styles. That wrapper is a block-level element by default, which can break layouts that expect the target to participate
  directly in a flex or grid container. `wrapperClass` is the escape hatch for those cases — common values are `display: contents`
  to make the wrapper layout-transparent, or `height: 100%` to let it stretch with its parent.
</Aside>

<Aside type="caution">
  `display: contents` removes the wrapper from the box tree, which historically caused some browsers (notably older
  Safari) to also drop it from the accessibility tree. Modern browsers handle this correctly for most elements, but if
  the wrapper sits around landmarks or interactive content, verify with a screen reader before shipping.
</Aside>
